...
/**
 * Test class of a catalog of reviews on an Hash Map
 */
class ReviewsHashMap implements ReviewsInterface, Serializable {
    /**
     * Reviews Catalog
     */
    Map<String, ReviewInterface> reviewsCatalog;

    /**
     * Empty Constructor
     */
    public ReviewsHashMap(){
        this.reviewsCatalog = new HashMap<>();
    }
     ...
     */
    public ReviewsHashMap(ReviewsHashMap r){
        this.setReviewsCatalog(r.getReviewsCatalog());
    }
     ...
     */
    public ReviewsHashMap clone(){
        return new ReviewsHashMap(this);
    }
     ...
     */
    public Map<String, ReviewInterface> getReviewsCatalog(){
        Map<String, ReviewInterface> ans = new HashMap<>();
        for(String s : this.reviewsCatalog.keySet())
            ans.put(s, this.reviewsCatalog.get(s).clone());
        return ans;
    }
     ...
     */
    public void setReviewsCatalog(Map<String, ReviewInterface> r){
        this.reviewsCatalog = new HashMap<>();
        for(String s : r.keySet())
            this.reviewsCatalog.put(s, r.get(s).clone());
    }
    ...
    public boolean existsReview(String ID){
        return reviewsCatalog.containsKey(ID);
    }
    ...
    public void addReview(ReviewInterface r){
        this.reviewsCatalog.put(r.getReview_ID(), r);
    }
    ...
    public Set<String> businessesReviewed(){
        ...
        for(ReviewInterface r : this.reviewsCatalog.values())
            ans.add(r.getBusiness_ID());
        return ans;
    }
    ...
    public int getNumberOfBusinessesReviewed(){
        Set<String> busisRevs = businessesReviewed();
        return busisRevs.size();
    }
    ...
    public List<Map.Entry<String, Integer>> query5(String userID, Map<String, String> idsAndNames){
        ...
        for(Map.Entry<String, ReviewInterface> e : this.reviewsCatalog.entrySet()){
            if(e.getValue().getUser_ID().equals(userID)){
                String busID = e.getValue().getBusiness_ID();
    ...
    /**
     * Method to get average grade of a given business ID
     * @param businessID Business ID
     * @return Average grade of the given business ID
     */
    public float getAverage(String businessID){
        float totalClass = 0;
        int totalRevs = 0;
        for(ReviewInterface r : this.reviewsCatalog.values())
            if(r.getBusiness_ID().equals(businessID)){
                totalClass += r.getStars();
                totalRevs++;
            }
        return totalClass / totalRevs;
    }
